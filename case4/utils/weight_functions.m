...
if constants.adaptive_peak_enabled
    ...
    for i = 1:length(unique_times)
        ...
        % Peak position is fixed at x=0.5 for analytical solution u(x,t) = x(1-x)*E_α(-t^α)
        expected_peak_x = 0.5;
...
% Enhanced peak region weights for better peak position learning

weights = ones(size(x_coords));

% Very precise peak region: x ∈ [0.45L, 0.55L] - maximum weight
precise_peak_mask = (x_coords > 0.45*L) & (x_coords < 0.55*L);
weights(precise_peak_mask) = 12.0;  % Increased maximum weight

% Peak region: x ∈ [0.4L, 0.6L] - high weight
peak_mask = (x_coords > 0.4*L) & (x_coords < 0.6*L) & ~precise_peak_mask;
...
weights(front_rear_mask) = 3.0;  % Reduced weight for boundary regions
...
% Numeric implementation of boundary region weights with adaptive detection

% Get constants for configurable parameters
constants = numerical_constants();
weights = ones(size(x_coords)) * constants.weights.interior_base;

if constants.boundary_adaptive_enabled
    % Adaptive boundary detection based on analytical solution gradients
    try
        % Sample analytical solution to determine actual boundary influence regions
        x_sample = linspace(0, params.L, 100);
        t_sample = mean(t_coords); % Use average time for boundary assessment
        u_sample = analytical_solution(x_sample, t_sample, params.alpha, params.D, params.v, params.N_terms);
        
        % Find regions where solution rapidly approaches zero (true boundary influence)
        du_dx = gradient(u_sample, x_sample);
        boundary_influence = abs(du_dx) > std(abs(du_dx)) * 2; % Regions with high gradient
        
        % Determine adaptive boundary thresholds
        boundary_indices = find(boundary_influence);
        if ~isempty(boundary_indices)
            left_boundary = x_sample(boundary_indices(1));
            right_boundary = x_sample(boundary_indices(end));
            boundary_mask = (x_coords < left_boundary) | (x_coords > right_boundary);
        else
            % Fallback to fixed threshold if adaptive detection fails
            boundary_mask = (x_coords < constants.boundary_detection_threshold*params.L) | ...
                          (x_coords > (1-constants.boundary_detection_threshold)*params.L);
        end
    catch
    ...
    % Fixed threshold boundary detection
    boundary_mask = (x_coords < constants.boundary_detection_threshold*params.L) | ...
                  (x_coords > (1-constants.boundary_detection_threshold)*params.L);
end

if any(boundary_mask)
    % Time-stratified boundary weighting using configurable weights
    t_vals = t_coords;
    
    % Get time thresholds from constants
    early_threshold = constants.early_time_threshold * params.T;
    late_threshold = constants.late_time_threshold * params.T;
    
    % Apply time-stratified weights
    early_time_mask = boundary_mask & (t_vals < early_threshold);
    weights(early_time_mask) = constants.weights.boundary_early;
    
    mid_time_mask = boundary_mask & (t_vals >= early_threshold) & (t_vals <= late_threshold);
    weights(mid_time_mask) = constants.weights.boundary_mid;
    
    late_time_mask = boundary_mask & (t_vals > late_threshold);
    weights(late_time_mask) = constants.weights.boundary_late;
end
...
t_vals = t_coords;
weights = 1 + 8 * (t_vals / T);  % Original time weighting

% Balanced emphasis on critical time regions
early_mask = t_vals < 0.3*T;
late_mask = t_vals > 0.7*T;

weights(early_mask) = weights(early_mask) * 1.5;  % Original early emphasis
weights(late_mask) = weights(late_mask) * 1.3;    % Original late emphasis
