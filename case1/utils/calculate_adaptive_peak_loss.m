...
if constants.adaptive_peak_enabled
    ...
    for i = 1:length(t_peak_samples)
        ...
        % 2. Path guidance: add supervision points along expected peak path
        % Expected peak position based on convection velocity with scaling
        x_expected = x_peak_initial + (params.v / params.D) * t * 0.05;
        
        % Ensure expected position is within domain bounds
        x_expected = max(0.1, min(0.9, x_expected));
